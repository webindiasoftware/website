...
async function seed() {
  ...
  const defaults = { home, services, products, navbarProducts, blog, productDetails }
  for (const [key, data] of Object.entries(defaults)) {
    await Content.findOneAndUpdate({ key }, { key, data }, { upsert: true, new: true })
  ...
  if (email && password) {
    ...
    console.warn('ADMIN_EMAIL / ADMIN_PASSWORD not set — skipped admin user seeding')
  }
