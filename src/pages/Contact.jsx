...
export default function Contact() {
  ...
  const [form, setForm] = useState({ name: '', email: '', sector: 'AEROSPACE & DEFENSE', urgency: 'standard', brief: '' })
  ...
  const handleSubmit = (e) => {
    ...
    dispatch({
      type: 'ADD_SUBMISSION',
      payload: { ...form, id: Date.now(), submittedAt: new Date().toISOString(), read: false },
    })
    setSubmitted(true)
    setForm({ name: '', email: '', sector: 'AEROSPACE & DEFENSE', urgency: 'standard', brief: '' })
    ...
    <main className="max-w-container-max mx-auto px-5 md:px-12 py-12">
      ...
      <header ref={heroRef} className="mb-16">
        ...
        <h1 className="font-headline-xl text-headline-lg-mobile md:text-headline-xl uppercase max-w-4xl">
          CONNECT WITH<br />ENGINEERING
        </h1>
        <p className="font-body-lg text-body-lg text-secondary mt-6 max-w-2xl">
          High-performance reliability meets precision infrastructure. Our team is standing by to integrate
          your next enterprise solution.
        </p>
      </header>
      ...
      <div ref={formRef} className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-4 space-y-6">
          <div className="border border-border-bold p-8 flex flex-col justify-between min-h-40 hover:bg-surface-muted transition-colors">
            <div className="flex items-center gap-4 mb-4">
              ...
              <span className="font-label-bold text-label-bold uppercase">Operations</span>
            </div>
          ...
          <div className="border border-border-bold p-8 flex flex-col justify-between min-h-40 hover:bg-surface-muted transition-colors">
            <div className="flex items-center gap-4 mb-4">
              ...
              <span className="font-label-bold text-label-bold uppercase">Direct Inquiry</span>
            </div>
          ...
          <div className="border border-border-bold p-8 flex flex-col justify-between min-h-56 hover:bg-surface-muted transition-colors">
            <div className="flex items-center gap-4 mb-4">
              ...
              <span className="font-label-bold text-label-bold uppercase">Headquarters</span>
            </div>
          ...
          <div className="bg-on-background text-on-primary p-8 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-label-bold text-label-bold uppercase">256-Bit Data Integrity</h3>
            </div>
            <p className="font-label-sm text-label-sm text-secondary-fixed-dim">
              All inquiries are handled via encrypted pipelines. Our 24/7 technical support ensures
              zero-latency response for critical infrastructure needs.
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-8 border border-border-bold p-8 md:p-12">
          <h2 className="font-headline-lg text-headline-lg-mobile md:text-headline-lg uppercase mb-12">Start Your Project</h2>
          ...
          <form className="space-y-8" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2">
                <label className="font-label-bold text-label-bold uppercase block">Industry Sector</label>
                <select name="sector" value={form.sector} onChange={handleChange} className="w-full bg-white border border-border-bold p-4 font-body-md text-body-md appearance-none cursor-pointer focus:outline-none focus:border-primary">
                  {['AEROSPACE & DEFENSE', 'FINTECH INFRASTRUCTURE', 'DATA ANALYTICS', 'SYSTEM ARCHITECTURE', 'HEALTHCARE', 'RETAIL & ECOMMERCE'].map((opt) => (
                    <option key={opt}>{opt}</option>
                  ))}
                </select>
              </div>
              <div className="space-y-2">
                <label className="font-label-bold text-label-bold uppercase block">Project Urgency</label>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-6 pt-4">
              <p className="text-secondary font-label-sm text-label-sm max-w-sm">
                By submitting, you agree to our strict data privacy and non-disclosure agreements.
              </p>
      ...
      <section ref={mapRef} className="mt-6">
        <div className="w-full h-[300px] md:h-[500px] border border-border-bold relative grayscale hover:grayscale-0 transition-all duration-700 overflow-hidden">
          ...
          <div className="absolute top-6 left-6 bg-on-background text-on-primary p-6 md:p-8 z-10 max-w-xs">
            <h4 className="font-label-bold text-label-bold uppercase mb-2">Technical Hub: Kolkata</h4>
            <p className="font-label-sm text-label-sm text-secondary-fixed-dim">Our primary engineering facility is located in the heart of Sector V, the region's technology epicenter.</p>
      ...
      <section ref={trustRef} className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-0 border border-border-bold">
        {[
          { title: '99.9% Uptime SLA', desc: 'Guaranteed operational continuity for all enterprise-level deployments and support cycles.' },
          { title: 'ISO 27001 Certified', desc: 'Global standards in information security management systems and data governance.' },
          { title: 'Global Network', desc: 'Strategically positioned engineering hubs to provide localized support across 14 time zones.' },
        ].map(({ title, desc }, i) => (
          <div key={title} className={`p-8 ${i < 2 ? 'border-b md:border-b-0 md:border-r border-border-bold' : ''}`}>
